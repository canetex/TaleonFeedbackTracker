-- Aumenta limites para 15 up / 15 down por IP e 15 por tipo na mesma sugestão.
-- Não altera nem remove votos existentes.

UPDATE public.config SET value = '15', updated_at = now()
WHERE key IN ('vote_limit_up', 'vote_limit_down', 'vote_limit_per_suggestion');
...
BEGIN
  ...
  v_limit_up := public._config_int('vote_limit_up', 15);
  v_limit_down := public._config_int('vote_limit_down', 15);
  v_limit_per_suggestion := public._config_int('vote_limit_per_suggestion', 15);
